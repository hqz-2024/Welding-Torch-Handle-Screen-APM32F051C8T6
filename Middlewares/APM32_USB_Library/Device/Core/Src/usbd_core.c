...
/*!
 * @brief     USB device get endpoint descriptor
 *
 ...
 * @retval    descriptor
 */
void *USBD_GetEpDesc(uint8_t *desc, uint8_t epAddr)
{
    USBD_DESC_HEADER_T *pDesc = (USBD_DESC_HEADER_T *)(void *)desc;
    USBD_CONFIG_DESC_T *pCfgDesc = (USBD_CONFIG_DESC_T *)(void *)desc;
    USBD_EP_DESC_T *pEpDesc = NULL;
    uint16_t ptr;

    if (pCfgDesc->wTotalLength > pCfgDesc->bLength)
    {
        ptr = pCfgDesc->bLength;
        while (ptr < pCfgDesc->wTotalLength)
        {
            pDesc = USBD_GetNextDesc((uint8_t *)pDesc, &ptr);

            if (pDesc->bDescriptorType == USBD_DESC_ENDPOINT)
            {
                pEpDesc = (USBD_EP_DESC_T *)(void *)pDesc;
...
/*!
 * @brief     USB device get next descriptor
 *
 * @param     buf : buffer
 *
 * @param     ptr : pointer
 ...
 * @retval    descriptor
 */
USBD_DESC_HEADER_T *USBD_GetNextDesc(uint8_t *buf, uint16_t *ptr)
{
    USBD_DESC_HEADER_T *pDesc = (USBD_DESC_HEADER_T *)(void *)buf;

    *ptr += pDesc->bLength;
    pDesc = (USBD_DESC_HEADER_T *)(void *)(buf + pDesc->bLength);
